...
// Added motion here to fix potential "not defined" errors
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
        ...
        <div className={`p-6 ${isCollapsed ? 'md:px-2' : ''}`}>
          <div className="flex items-center justify-between pb-6 mb-8 border-b border-white/10 md:flex-col md:gap-4">
            <div className="relative flex justify-center w-full">
              {/* Image logic: Favicon for collapsed, Header for expanded. No hover change. */}
              {isCollapsed ? (
                <img 
                  src="/favicon.ico" 
                  alt="Icon" 
                  className="w-10 h-10 p-1 bg-white rounded-lg shadow-md"
                />
              ) : (
                <img 
                  src="/header.webp" 
      ...
      <main className="relative flex-1 overflow-y-auto">
        {/* Mobile Header (Hamburger Menu visible here) */}
        <header className="flex items-center justify-between p-6 bg-white border-b md:hidden">
            <div className="flex items-center gap-3">
                <button 
                  onClick={() => setIsMobileOpen(true)}
                  className="p-2 -ml-2 transition-colors rounded-lg hover:bg-gray-100 text-brand-red"
                >
                  <Menu size={24} />
                </button>
